extends RefCounted
class_name MixerRecipeCatalog

const DEFINITIONS_PATH := "res://data/stand_mixer_recipe_definitions.json"
...
static func _ensure_loaded() -> void:
	...
	if file == null:
		push_error("Stand mixer recipes are missing: %s" % DEFINITIONS_PATH)
		return
	var parsed: Variant = JSON.parse_string(file.get_as_text())
	if not parsed is Dictionary:
		push_error("Stand mixer recipes contain invalid JSON: %s" % DEFINITIONS_PATH)
		return
	...
	for recipe_id_value: Variant in recipe_ids:
		...
		if not inputs_value is Array or (inputs_value as Array).is_empty() or (inputs_value as Array).size() > 3 \
			...
			push_warning("Skipped invalid stand mixer recipe: %s" % str(recipe_id_value))
			continue
		...
		for input_value: Variant in inputs_value:
			...
			if input_id.is_empty() or float(input.get("weight_kg", 0.0)) <= 0.0 \
				...
				break
		if not valid_inputs:
			push_warning("Skipped invalid stand mixer recipe inputs: %s" % str(recipe_id_value))
			continue
		...
		if absf(output_weight / pickup_unit - roundf(output_weight / pickup_unit)) > 0.0001:
			push_warning("Stand mixer output must be a pickup-unit multiple: %s" % str(recipe_id_value))
			continue
