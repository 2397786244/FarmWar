extends Node3D
class_name MedicinePistolTool

@export var tool_owner := ""
@export var bullet_speed := 48.0

const BULLET_SCENE := preload("res://character/weapons/MedicineBullet.tscn")
...
func _emit_bullet(visual_only: bool, direction_override := Vector3.ZERO) -> void:
	...
	if tool_owner.is_empty() or not is_instance_valid(GlobalVar.gameworld):
		return
	var bullet := BULLET_SCENE.instantiate() as MedicineBullet
	if bullet == null:
		return
	if visual_only:
...
func _get_center_screen_direction(shooter: CollisionObject3D) -> Vector3:
	...
	var max_distance := CombatBalance.get_float("medicine_pistol", "range", 60.0)
...
func _play_recoil() -> void:
	...
	var recoil_offset := CombatBalance.get_model_recoil_offset("medicine_pistol")
