...
func _cache_van_nodes() -> void:
	_headlight_glows.clear()
	_brake_glows.clear()

	var mesh := get_node_or_null(MESH_NODE_NAME) as Node3D
	if mesh == null:
		push_error("Van: missing direct Mesh visual node.")
		return
	...
	for node_name: String in HEADLIGHT_GLOW_NAMES:
		...
		if glow != null:
			...
			push_warning("Van: Mesh is missing %s." % node_name)
