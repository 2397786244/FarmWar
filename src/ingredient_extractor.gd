...
class_name IngredientExtractor
...
func _ready() -> void:
	add_to_group("ingredient_extractors")
	_refresh_visual()


func _process(_delta: float) -> void:
	if GameAuthority.is_client_proxy() or not cooking:
		return
	var now_msec := Time.get_ticks_msec()
	if get_progress(now_msec) >= 1.0:
		cooking = false
		complete = true
		# The output is now shared and can be collected by any teammate.
...
func interact(player: GamePlayer) -> bool:
	...
	if cooking:
		return false
	var page := player.get_node_or_null("SubViewport/IngredientExtractorPage")
...
func get_display_inputs() -> Array[Dictionary]:
	return staged_inputs.duplicate(true)


func can_start_extraction(next_recipe_id: String) -> bool:
	return recipe_id.is_empty() and not cooking and not complete \
		and not ExtractorRecipeCatalog.get_recipe(next_recipe_id).is_empty()


func start_extraction(next_recipe_id: String) -> bool:
	if not can_start_extraction(next_recipe_id):
		return false
	var recipe := ExtractorRecipeCatalog.get_recipe(next_recipe_id)
	recipe_id = next_recipe_id
	staged_inputs.clear()
	for input: Dictionary in ExtractorRecipeCatalog.get_inputs(next_recipe_id):
...
func get_extractor_state() -> Dictionary:
	var state := {
		...
		"progress": get_progress(Time.get_ticks_msec()),
	}
	state.merge(get_user_lock_state(), true)
	return state


func apply_authoritative_extractor_state(state: Dictionary) -> void:
...
func _emit_authoritative_state() -> void:
	GameAuthority.reliable_world_event_ready.emit({
		...
		"station_state": get_extractor_state(),
...
func _refresh_visual() -> void:
	...
	if not is_instance_valid(status_label):
		return
	if complete:
		status_label.text = "按 [E] 拿取提取产物"
	elif cooking:
		status_label.text = "提取中 %d%%" % roundi(progress * 100.0)
	elif recipe_id.is_empty():
		status_label.text = "按 [E] 使用食材提取器"
	else:
		status_label.text = "放入原料后开始提取"
