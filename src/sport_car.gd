...
func set_drive_input(throttle: float, steering: float, brake: float = 0.0) -> void:
	...
	# VehicleBase holds an empty vehicle with a full brake input. Only show the
	# rear lamps when a driver is actually braking.
...
func _cache_sport_car_nodes() -> void:
	_headlight_glows.clear()
	_brake_glows.clear()

	var mesh := get_node_or_null(MESH_NODE_NAME) as Node3D
	if mesh == null:
		push_error("SportCar: missing direct Mesh visual node.")
		return
	...
	for node_name: String in HEADLIGHT_GLOW_NAMES:
		...
		if glow != null:
			...
			push_warning("SportCar: Mesh is missing %s." % node_name)
