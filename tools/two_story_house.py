...
#
# Run:
#   blender --background --factory-startup --python generate_farmtown_two_story_house_ramp.py
# ---------------------------------------------------------------------
...
OUTPUT_FILE = "FTF_House_NorthAmerican_TwoStory_Ramp.glb"

ROOT_NAME = "FTF_House_NorthAmerican_TwoStory_Ramp"
...
TARGET_SECOND_FLOOR_Z = FOUNDATION_H + FIRST_WALL_H  # finished floor height
...
def build_house():
    ...
    # Second floor slab with ramp opening
    upper_z0 = FOUNDATION_H + FIRST_WALL_H
    upper_z1 = upper_z0 + UPPER_FLOOR_T

    # Build second floor as 3 slabs around ramp hole / landing void
    ...
    # Ramp / landing zone on right-rear quadrant
    shaft_x0 = 0.35
    shaft_x1 = clear_right - 0.45
    shaft_y0 = -1.20
    shaft_y1 = clear_back - 0.45
    ...
    # Front-right slab
    add_cube("UpperFloor_FrontRight",
             ((shaft_x0 + shaft_x1) * 0.5, (clear_front + shaft_y0) * 0.5, (upper_z0 + upper_z1) * 0.5),
             (shaft_x1 - shaft_x0, shaft_y0 - clear_front, UPPER_FLOOR_T),
             mats["floor"], root, collection, bevel=0.004)
    # Rear strip
    add_cube("UpperFloor_RearStrip",
    ...
    second_z = upper_z0 + UPPER_FLOOR_T
    front_openings_2 = [
    ...
    # Ramp 1: runs along +X from front-left interior toward landing
    ramp1_start_x = clear_left + 0.55
    ramp1_end_x = ramp1_start_x + RAMP_RUN_1
    ramp1_y = clear_front + 1.55

    add_cube("Ramp_1",
             ((ramp1_start_x + ramp1_end_x) * 0.5, ramp1_y, FOUNDATION_H + RAMP_RISE_1 * 0.5),
             (math.sqrt(RAMP_RUN_1 ** 2 + RAMP_RISE_1 ** 2), RAMP_WIDTH, RAMP_THICKNESS),
             mats["porch"], root, collection, bevel=0.006,
             rotation=(0.0, -math.atan2(RAMP_RISE_1, RAMP_RUN_1), 0.0))

    landing_x = ramp1_end_x + LANDING_W * 0.5
    landing_y = ramp1_y + (LANDING_D * 0.5 - RAMP_WIDTH * 0.5)
    add_cube("RampLanding",
             (landing_x, landing_y, HALF_LEVEL_Z + RAMP_THICKNESS * 0.5),
             (LANDING_W, LANDING_D, RAMP_THICKNESS),
             mats["porch"], root, collection, bevel=0.006)

    # Ramp 2: turns 90 degrees, goes toward +Y to reach second floor
    ramp2_start_y = landing_y + LANDING_D * 0.5 - RAMP_WIDTH * 0.5
    ramp2_end_y = ramp2_start_y + RAMP_RUN_2
    ramp2_x = landing_x + (LANDING_W * 0.5 - RAMP_WIDTH * 0.5)

    add_cube("Ramp_2",
             (ramp2_x, (ramp2_start_y + ramp2_end_y) * 0.5, HALF_LEVEL_Z + RAMP_RISE_2 * 0.5),
             (RAMP_WIDTH, math.sqrt(RAMP_RUN_2 ** 2 + RAMP_RISE_2 ** 2), RAMP_THICKNESS),
             mats["porch"], root, collection, bevel=0.006,
             rotation=(math.atan2(RAMP_RISE_2, RAMP_RUN_2), 0.0, 0.0))

    # Upper landing platform where ramp 2 arrives
    upper_landing_center_x = ramp2_x
    upper_landing_center_y = min(clear_back - 0.95, ramp2_end_y + 0.60)
    add_cube("UpperLanding",
             (upper_landing_center_x, upper_landing_center_y, TARGET_SECOND_FLOOR_Z + RAMP_THICKNESS * 0.5),
             (RAMP_WIDTH + 0.45, 1.20, RAMP_THICKNESS),
             mats["porch"], root, collection, bevel=0.006)
