...
ASSET_NAME = "FTF_Resource_CopperOre_1m"
...
def add_cylinder_between(root, name, start, end, radius, material, vertices=8, bevel=0.0):
    ...
    if length <= 0.0001:
        raise ValueError("Cylinder endpoints must differ for " + name)
...
def add_cone_between(root, name, start, end, radius_start, radius_end, material, vertices=8, bevel=0.0):
    start_v = Vector(start)
    end_v = Vector(end)
    direction = end_v - start_v
    length = direction.length
    if length <= 0.0001:
        raise ValueError("Cone endpoints must differ for " + name)
...
def add_mesh_object(root, name, vertices, faces, material, bevel=0.0):
...
def validate_visual_asset(root, expected_height=None, height_tolerance=0.10, require_bottom_at_zero=False):
    ...
    if require_bottom_at_zero and min_v.z < -0.002:
...
def create_preview_scene(root):
    ...
    bpy.ops.mesh.primitive_plane_add(size=span * 3.5, location=(center.x, center.y, min_v.z - 0.020))
    ground = bpy.context.active_object
    ground.name = "PreviewGround"
    ground.data.materials.append(ground_material)

    bpy.ops.object.camera_add(location=(center.x + span * 1.45, center.y - span * 1.62, min_v.z + span * 1.02))
...
def main():
    ...
    validate_visual_asset(root, expected_height=EXPECTED_HEIGHT, height_tolerance=HEIGHT_TOLERANCE, require_bottom_at_zero=REQUIRE_BOTTOM_AT_ZERO)
...
def build_irregular_boulder(root, rock_material):
    # Fixed, hand-authored low-poly ring profile: no random seed required.
    bottom = [
        (-0.38, -0.30, 0.00), (-0.10, -0.43, 0.00), (0.27, -0.36, 0.00), (0.43, -0.10, 0.00),
        (0.36, 0.28, 0.00), (0.05, 0.42, 0.00), (-0.30, 0.34, 0.00), (-0.46, 0.05, 0.00),
    ]
    middle = [
        (-0.47, -0.26, 0.36), (-0.14, -0.52, 0.38), (0.32, -0.39, 0.42), (0.52, -0.09, 0.39),
        (0.40, 0.32, 0.37), (0.02, 0.49, 0.42), (-0.34, 0.40, 0.39), (-0.55, 0.06, 0.35),
    ]
    upper = [
        (-0.26, -0.18, 0.72), (-0.05, -0.29, 0.79), (0.21, -0.22, 0.76), (0.31, -0.02, 0.80),
        (0.22, 0.22, 0.75), (0.00, 0.29, 0.82), (-0.22, 0.23, 0.78), (-0.31, 0.00, 0.75),
    ]
    top = (0.01, -0.02, 1.00)
    vertices = bottom + middle + upper + [top]
    faces = []
    for i in range(8):
        j = (i + 1) % 8
        faces.append((i, j, 8 + j, 8 + i))
        faces.append((8 + i, 8 + j, 16 + j, 16 + i))
        faces.append((16 + i, 16 + j, 24))
    # A bottom face is not visible during normal gameplay but closes the mesh; it is not coplanar with any separate surface.
    faces.append(tuple(range(7, -1, -1)))
    return add_mesh_object(root, "OreRockBody", vertices, faces, rock_material, bevel=0.010)


def add_ore_chunk(root, name, center, scale, material, rotation=(0.0, 0.0, 0.0)):
    bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=1, radius=1.0, location=center, rotation=rotation)
    obj = bpy.context.active_object
    obj.name = name
    obj.scale = scale
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
...
def build_asset():
    ...
    rock = make_material("CopperOreRock", "#53636A", roughness=0.88, metallic=0.0)
    ore_main = make_material("CopperOreVein", "#C96F3B", roughness=0.48, metallic=0.25)
    ore_highlight = make_material("CopperOreHighlight", "#E5A367", roughness=0.43, metallic=0.20)

    build_irregular_boulder(root, rock)

    # Veins protrude about 0.01-0.04m into the boulder, avoiding accidental floating parts.
    add_ore_chunk(root, "OreVeinFront", (0.04, -0.455, 0.42), (0.20, 0.045, 0.15), ore_main, rotation=(0.35, 0.10, 0.18))
    add_ore_chunk(root, "OreVeinUpper", (-0.18, -0.12, 0.75), (0.16, 0.12, 0.13), ore_highlight, rotation=(0.15, -0.25, 0.65))
    add_ore_chunk(root, "OreVeinRight", (0.405, 0.08, 0.48), (0.045, 0.16, 0.13), ore_main, rotation=(0.35, 0.45, -0.28))
    add_ore_chunk(root, "OreVeinLeft", (-0.39, 0.13, 0.34), (0.050, 0.13, 0.11), ore_highlight, rotation=(0.20, -0.35, 0.40))
    add_ore_chunk(root, "OreVeinTop", (0.05, 0.12, 0.90), (0.12, 0.09, 0.065), ore_main, rotation=(0.20, 0.15, -0.45))
    return root
