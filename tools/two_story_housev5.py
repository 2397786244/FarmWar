...
#
# Run:
#   blender --background --factory-startup --python generate_farmtown_two_story_house_ramp_v5.py
# ---------------------------------------------------------------------
...
OUTPUT_FILE = "FTF_House_NorthAmerican_TwoStory_Ramp_v5.glb"

ROOT_NAME = "FTF_House_NorthAmerican_TwoStory_Ramp_v5"
...
GROUND_WALK_Z = FOUNDATION_H + GROUND_FLOOR_T
SECOND_WALK_Z = FOUNDATION_H + FIRST_WALL_H + UPPER_FLOOR_T  # finished second-floor walking surface
HALF_LEVEL_Z = GROUND_WALK_Z + (SECOND_WALK_Z - GROUND_WALK_Z) * 0.5
TARGET_SECOND_FLOOR_Z = SECOND_WALK_Z
...
def add_ramp_wedge(name, axis, center_xy, run_length, width, z_start, z_end, thickness, material, parent, collection):
    """
    Build a proper wedge ramp mesh with a continuous top surface.
    This avoids fake-looking stepped gaps from rotated boxes.

    axis:
      "+Y", "-Y", "+X", "-X"
    center_xy:
      center of the ramp footprint in XY
    """
    hx = width * 0.5
    hy = run_length * 0.5
    ...
    if axis in ("+Y", "-Y"):
        ...
        verts = [
            ...
            (x0, y0, z_near - thickness), (x1, y0, z_near - thickness),
            (x1, y1, z_far - thickness), (x0, y1, z_far - thickness),
        ]
    else:
        x0 = -hy
        x1 = hy
        ...
        verts = [
            ...
            (x0, y0, z_left - thickness), (x1, y0, z_right - thickness),
            (x1, y1, z_right - thickness), (x0, y1, z_left - thickness),
        ]
    ...
    obj = bpy.data.objects.new(name, mesh)
    collection.objects.link(obj)
    obj.location = (center_xy.x, center_xy.y, 0.0)
    assign_material(obj, material)
    set_flat_shading(obj)
    add_bevel(obj, 0.004)
    obj.parent = parent
...
def build_house():
    ...
    # Second floor slab with a large clean opening for the right-side ramp system.
    # This void prevents the upper floor from clipping the stair/ramp arrival.
    ...
    shaft_x0 = -1.55
    shaft_x1 = clear_right - 0.12
    shaft_y0 = clear_front + 0.18
    shaft_y1 = 2.30
    ...
    porch_roof_depth = PORCH_D + 0.30
    add_cube("PorchRoof",
             (0.0, front - porch_roof_depth * 0.5 + 0.02, FOUNDATION_H + PORCH_POST_H + 0.08),
             (PORCH_W + 0.32, porch_roof_depth, 0.16),
             mats["roof"], root, collection, bevel=0.008)
    add_cube("PorchRoofLedger",
             (0.0, front + 0.04, FOUNDATION_H + PORCH_POST_H + 0.08),
             (PORCH_W + 0.18, 0.12, 0.20),
    ...
    # use real wedge ramps with exact walk-surface heights.
    # This keeps every transition tightly connected without fake seams.
    #
    # Layout:
    # - ramp on the RIGHT of the front door
    # - first ramp goes FORWARD (+Y)
    # - half-level landing
    # - second ramp turns LEFT (-X) to the second floor
    ramp1_x = clear_right - 1.10
    ramp1_start_y = clear_front + 0.62
    ramp1_end_y = ramp1_start_y + RAMP_RUN_1

    add_ramp_wedge(
        "Ramp_1",
        "+Y",
        Vector((ramp1_x, (ramp1_start_y + ramp1_end_y) * 0.5, 0.0)),
        RAMP_RUN_1,
        RAMP_WIDTH,
        GROUND_WALK_Z,
        HALF_LEVEL_Z,
        RAMP_THICKNESS,
        mats["porch"], root, collection
    )

    landing_x = clear_right - LANDING_W * 0.5 - 0.18
    ...
    ramp2_end_x = shaft_x0 + 0.92
    ramp2_start_x = landing_x - LANDING_W * 0.5
    ramp2_center_x = (ramp2_start_x + ramp2_end_x) * 0.5
    ramp2_y = landing_y

    add_ramp_wedge(
        "Ramp_2",
        "-X",
        Vector((ramp2_center_x, ramp2_y, 0.0)),
        RAMP_RUN_2,
        RAMP_WIDTH,
        HALF_LEVEL_Z,
        SECOND_WALK_Z,
    ...
    # Upper landing / arrival platform at the second floor.
    upper_landing_center_x = ramp2_end_x - 0.55
    upper_landing_center_y = ramp2_y
    add_cube("UpperLanding",
             (upper_landing_center_x, upper_landing_center_y, SECOND_WALK_Z - RAMP_THICKNESS * 0.5),
             (1.10, RAMP_WIDTH + 0.35, RAMP_THICKNESS),
             mats["porch"], root, collection, bevel=0.006)

    # User-requested change:
    # remove all upper-floor guardrails so they do not block the stair/ramp exit.
    # The ramp and landing geometry remain unchanged.
...
if __name__ == "__main__":
    ...
    print(" - Upper-floor guardrails removed to keep the stair/ramp exit clear.")
