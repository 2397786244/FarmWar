...
ASSET_NAME = "FTF_Plant_SugarCane_Single_3m"
...
# Food War plant asset: 3m single sugar cane for planting.
EXPECTED_HEIGHT = 3.0
...
def build_asset():
    ...
    stalk_light = make_material("CaneStalkLight", "#78C95B", roughness=0.82)
    stalk_dark = make_material("CaneStalkShade", "#4F8A45", roughness=0.84)
    node_material = make_material("CaneNodeBand", "#35643A", roughness=0.88)
    leaf_light = make_material("CaneLeafLight", "#63A94E", roughness=0.86)
    leaf_dark = make_material("CaneLeafShade", "#35643A", roughness=0.90)

    # A chunky segmented cane stalk; all parts physically meet at the node bands.
    segments = [
        (0.325, 0.65, stalk_light),
        (1.035, 0.65, stalk_dark),
        (1.745, 0.65, stalk_light),
        (2.455, 0.65, stalk_dark),
    ]
    for index, (z, height, material) in enumerate(segments, start=1):
        add_cylinder(
            root, "CaneStalkSegment{:02d}".format(index), (0.0, 0.0, z),
            radius=0.105, depth=height, material=material, vertices=8, bevel=0.012
        )

    for index, z in enumerate((0.680, 1.390, 2.100, 2.810), start=1):
        add_cylinder(
            root, "CaneNodeBand{:02d}".format(index), (0.0, 0.0, z),
            radius=0.121, depth=0.060, material=node_material, vertices=8, bevel=0.006
        )

    # Broad low-poly leaves create a readable sugar-cane silhouette at gameplay distance.
    leaf_specs = [
        ((0.00, 0.00, 0.38), (1.0, 0.20), 0.88, 0.115, 0.24, 0.12, leaf_dark),
        ((0.00, 0.00, 0.62), (-0.35, 1.0), 0.82, 0.110, 0.22, 0.15, leaf_light),
        ((0.00, 0.00, 1.05), (-1.0, 0.15), 0.75, 0.105, 0.20, 0.12, leaf_dark),
        ((0.00, 0.00, 1.48), (0.22, -1.0), 0.70, 0.095, 0.20, 0.13, leaf_light),
        ((0.00, 0.00, 2.14), (0.85, -0.55), 0.54, 0.080, 0.17, 0.12, leaf_dark),
        # Top leaves remain within the strict 3m total height.
        ((0.00, 0.00, 2.72), (-0.58, 0.82), 0.42, 0.070, 0.34, 0.10, leaf_light),
    ]
    for index, spec in enumerate(leaf_specs, start=1):
        base, direction, length, width, arch, drop, material = spec
        add_tapered_leaf(
            root, "CaneLeaf{:02d}".format(index), base, direction, length,
            width, arch, drop, material, thickness=0.010, segments=3
        )
