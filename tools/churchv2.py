# Blender 4.x / 5.x
# ---------------------------------------------------------------------
# Farm Town - Large Two-Story Rural Church (Fixed Variants)
#
# This script exports TWO fixed church variants:
#   1) FTF_RuralChurch_Large_TwoStory_Light.glb
#   2) FTF_RuralChurch_Large_TwoStory_DarkWood.glb
#
# FIX SUMMARY
# - Rechecked side facade layout carefully.
# - Moved nave side windows so they do NOT overlap:
#     * transept cross-wing volume
#     * side annex volumes
# - Moved nave side buttresses to non-overlapping exposed wall segments.
# - Kept side doors only on exposed transept side faces.
# - Preserved large, complex North-American rural church silhouette.
# - Non-enterable static building.
#
# COMMON ASSET CONTRACT
# - Front faces local -Y
# - Root at ground-center (0, 0, 0)
# - No interior
# - No yard
# - No collision shell
...
#
# Run:
#   blender --background --factory-startup --python generate_farmtown_rural_church_fixed_variants.py
# ---------------------------------------------------------------------
...
VARIANTS = [
    {
        "name": "FTF_RuralChurch_Large_TwoStory_Light",
        "wall": (0.85, 0.84, 0.79),
        "wall_shadow": (0.74, 0.73, 0.68),
        "trim": (0.92, 0.90, 0.85),
        "steeple": (0.84, 0.83, 0.80),
    },
    {
        "name": "FTF_RuralChurch_Large_TwoStory_DarkWood",
        "wall": (0.33, 0.22, 0.15),
        "wall_shadow": (0.24, 0.16, 0.11),
        "trim": (0.77, 0.72, 0.63),
        "steeple": (0.66, 0.61, 0.54),
    },
]
...
def build_materials(variant_name, palette):
    return {
        "foundation": make_material(f"MAT_{variant_name}_Foundation", (0.31, 0.31, 0.30), 0.88),
        "wall": make_material(f"MAT_{variant_name}_Wall", palette["wall"], 0.82),
        "wall_shadow": make_material(f"MAT_{variant_name}_WallShadow", palette["wall_shadow"], 0.85),
        "trim": make_material(f"MAT_{variant_name}_Trim", palette["trim"], 0.72),
        "roof": make_material(f"MAT_{variant_name}_Roof", (0.43, 0.17, 0.11), 0.74),
        "roof_dark": make_material(f"MAT_{variant_name}_RoofDark", (0.13, 0.12, 0.11), 0.76),
        "door": make_material(f"MAT_{variant_name}_Door", (0.18, 0.10, 0.055), 0.80),
        "window": make_material(f"MAT_{variant_name}_Window", (0.10, 0.16, 0.22), 0.34, 0.03),
        "steeple": make_material(f"MAT_{variant_name}_Steeple", palette["steeple"], 0.78),
        "accent": make_material(f"MAT_{variant_name}_Accent", (0.57, 0.47, 0.31), 0.82),
    }
...
def add_cube(
    name, location, dimensions, material, parent, collection,
    bevel=0.0, rotation=(0.0, 0.0, 0.0)
):
...
def add_cylinder(
    name, location, radius, depth, material, parent, collection,
    vertices=12, rotation=(0.0, 0.0, 0.0), bevel=0.0
):
...
def add_cone(
    name, location, radius1, depth, material, parent, collection,
    vertices=8, rotation=(0.0, 0.0, 0.0), bevel=0.0
):
    bpy.ops.mesh.primitive_cone_add(
        vertices=vertices, radius1=radius1, radius2=0.0,
...
def add_gable_triangle(
    name, center_x, center_y, thickness_y, width_x,
...
def add_round_window(name, x, y, z, diameter, mats, parent, collection, facing="front"):
    if facing in ("front", "back"):
        add_cylinder(name + "_Pane", (x, y, z), diameter * 0.5, WINDOW_PANEL_T,
...
def create_root(collection, root_name):
...
def validate_layout_before_build():
    """
    Explicitly verify the exposed nave side segments so we do not place windows
    or buttresses inside the annex or transept overlap zones.
    """
    ...
    # The exposed side segments of nave are approximately:
    # 1) from nave_front to annex_front
    # 2) from tran_back to nave_back
    ...
    if exposed_front_len < 2.0 or exposed_back_len < 2.0:
        raise RuntimeError("Church layout invalid: exposed nave side segments became too small.")

    print(
        "[LAYOUT OK]\n"
        f"  Nave front: {nave_front:.2f}, back: {nave_back:.2f}\n"
        f"  Annex coverage on nave sides: {annex_front:.2f}..{annex_back:.2f}\n"
        f"  Transept coverage on nave sides: {tran_front:.2f}..{tran_back:.2f}\n"
        f"  Exposed front-side segment length: {exposed_front_len:.2f}m\n"
        f"  Exposed back-side segment length:  {exposed_back_len:.2f}m\n"
    )


def build_church_variant(variant):
    ...
    root_name = variant["name"]
    collection = get_or_create_collection("COL_" + root_name)
    mats = build_materials(root_name, variant)
    root = create_root(collection, root_name)
    ...
    add_closed_door_side("Door_SideLeft", tran_left - 0.06, 1.05,
                         FOUNDATION_H, SIDE_DOOR_W, SIDE_DOOR_H, mats, root, collection)
    add_closed_door_side("Door_SideRight", tran_right + 0.06, 1.05,
                         FOUNDATION_H, SIDE_DOOR_W, SIDE_DOOR_H, mats, root, collection)
    ...
    # Windows
    # ---------------------------------------------------------
    # Front tower / front nave windows
    ...
    add_side_window("Tower_Belfry_Left", tower_cx - TOWER_W * 0.5 - 0.05, tower_cy,
                    FOUNDATION_H + 8.55, 0.95, 1.35, mats, root, collection)
    ...
    # FIXED nave side windows:
    # Only place on truly exposed nave side segments:
    # front exposed segment before annex starts, and back exposed segment after transept ends.
    nave_exposed_window_ys = [-7.45, 5.35, 8.15]
    for i, y in enumerate(nave_exposed_window_ys):
        add_side_window(f"NaveLeftWindow_Lower_{i}", nave_left - 0.05, y,
                        FOUNDATION_H + 1.90, LANCET_W, LANCET_H, mats, root, collection)
        add_side_window(f"NaveRightWindow_Lower_{i}", nave_right + 0.05, y,
                        FOUNDATION_H + 1.90, LANCET_W, LANCET_H, mats, root, collection)

        add_side_window(f"NaveLeftWindow_Upper_{i}", nave_left - 0.05, y,
                        FOUNDATION_H + 4.95, UPPER_W, UPPER_H, mats, root, collection)
        add_side_window(f"NaveRightWindow_Upper_{i}", nave_right + 0.05, y,
                        FOUNDATION_H + 4.95, UPPER_W, UPPER_H, mats, root, collection)

    # Transept front/back windows
    tran_window_xs = [-5.8, 5.8]
    for i, x in enumerate(tran_window_xs):
    ...
    for side, cx in (("Left", annex_l_cx), ("Right", annex_r_cx)):
        ...
        add_front_window(f"AnnexFrontWindow_{side}", cx, y, FOUNDATION_H + 1.85,
                         1.15, 1.55, mats, root, collection)

    # ---------------------------------------------------------
    # Buttresses - also moved to exposed segments only.
    # ---------------------------------------------------------
...
def validate_asset(root):
    ...
    print(
        ...
        f"  Note: nave side windows / buttresses were relocated to exposed wall segments only.\n"
    )
...
if __name__ == "__main__":
    print("\n=== Generating Fixed Rural Church Variants ===\n")

    output_paths = []

    for variant in VARIANTS:
        print(f"\n--- Building variant: {variant['name']} ---\n")
        root = build_church_variant(variant)
        ...
        output_path = os.path.join(OUTPUT_DIR, variant["name"] + ".glb")
    ...
    for path in output_paths:
        print(" - " + path)
    print("\nNotes:")
    print(" - This script exports both the light church and the dark wood church.")
    print(" - The side-window / side-wall / side-door overlap issue was corrected.")
