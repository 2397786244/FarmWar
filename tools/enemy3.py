...
DEFAULT_OUTPUT = os.path.join(SCRIPT_DIR, "generated_foodwar_enemies")
FPS = 30
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate rigged Food War enemy villain characters with animations, GLB export, and pose checks."
    )
...
def create_role_materials(role_name, team_name):
    ...
    role_palettes = {
        ...
        "MaskedBandit": {
            # Brown skin, dark hair, urban stealth outfit with burgundy accent cloth.
            "skin": ("#8A5A3B", 0.0, 0.76),
            "hair": ("#251E1D", 0.0, 0.68),
            "primary": ("#2B2F37", 0.0, 0.68),
            "accent": ("#7E2830", 0.0, 0.66),
        },
        "BruiserRider": {
            # Tall, stocky brown-skinned bald bruiser with rugged rider gear.
            "skin": ("#8B5B3B", 0.0, 0.76),
            "hair": ("#4C3529", 0.0, 0.70),
            "primary": ("#4A4139", 0.0, 0.66),
            "accent": ("#8C5A2B", 0.0, 0.62),
        },
        "FutureSoldier": {
            # Black/navy armor with a restrained electric-blue technology accent.
            # The Enemy team material remains red for other enemies, but this role
            # deliberately does not use it anywhere in its visible equipment.
            "skin": ("#D5A27E", 0.0, 0.76),
            "hair": ("#1A1E25", 0.0, 0.68),
            "primary": ("#10283C", 0.10, 0.46),
            "accent": ("#1A78B8", 0.15, 0.38),
        },
    }
    colors = dict(common)
...
def build_prospector(team_name):
    ...
    add_cylinder("MinerLampBand", (0.0, -0.185, 1.865), 0.21, 0.036, mats["dark"], root, "Head", vertices=10, rotation=(math.radians(90), 0, 0))
    add_cube("MinerLamp", (0.0, -0.225, 1.870), (0.13, 0.050, 0.080), mats["team"], root, "Head", bevel=0.010)
...
def hide_bandit_lower_face_features(root):
    """Keep the generic facial meshes for blend-shape validation, but bury the
    nose and mouth safely inside the head because the cloth mask owns the lower
    face silhouette. This avoids both visible nose/mouth clipping and a floating
    mask that has to sit far in front of the face.
    """
    hidden_names = ("Nose", "MouthLeft", "MouthCenter", "MouthRight", "FunnyTongue")
    for child in root.children:
        if child.name in hidden_names:
            child.location.y = 0.055  # inside the head volume; not visible from outside


def remove_standard_brows(root):
    """The bald Bruiser uses one heavy brow guard instead of separate eyebrows.
    Removing only these two visual meshes retains eye/mouth blend-shape meshes.
    """
    for child in list(root.children):
        if child.name in {"LeftBrow", "RightBrow"}:
            bpy.data.objects.remove(child, do_unlink=True)


def replace_mesh_material(root, mesh_name, material):
    """Replace a generated mesh material without removing its facial shape keys."""
    for child in root.children:
        if child.name == mesh_name and child.type == "MESH":
            child.data.materials.clear()
            child.data.materials.append(material)
            return
    raise RuntimeError("Cannot replace material on missing mesh: " + mesh_name)


def build_masked_bandit(team_name):
    root = create_root("FTF_Enemy_MaskedBandit")
    mats = create_role_materials("MaskedBandit", team_name)
    add_female_body(root, mats, mats["primary"], mats["dark"], mats["dark"], sleeve_material=mats["primary"], cute=False)
    hide_bandit_lower_face_features(root)

    # Close outer clothing: all visual layers either intersect their parent piece
    # by 2-5 mm or are bridged with a dedicated mounting component.
    add_cube("BanditJacketFront", (0.0, -0.184, 1.18), (0.50, 0.040, 0.44), mats["primary"], root, "Chest", bevel=0.032)
    add_cube("BanditJacketZip", (0.0, -0.214, 1.18), (0.028, 0.012, 0.38), mats["accent"], root, "Chest", bevel=0.004)
    add_cube("BanditLeftSleevePatch", (-0.452, 0.000, 1.24), (0.060, 0.22, 0.070), mats["accent"], root, "UpperArm.R", bevel=0.010, rotation=(math.radians(28), 0.0, math.radians(-20)))
    add_cube("BanditRightSleevePatch", (0.452, 0.000, 1.24), (0.060, 0.22, 0.070), mats["accent"], root, "UpperArm.L", bevel=0.010, rotation=(math.radians(28), 0.0, math.radians(20)))
    add_open_box_band_shell("BanditWaistBelt", 0.90, 0.235, 0.154, 0.085, mats["dark"], root, "Hips")
    # Belt front is y=-0.154. The buckle rear is pushed 2 mm into that belt
    # surface, which makes it read as sewn/mounted rather than floating 8 mm ahead.
    add_cube("BanditBuckle", (0.0, -0.160, 0.90), (0.11, 0.016, 0.075), mats["accent"], root, "Hips", bevel=0.010)
    add_cube("BanditLeftBootWrap", (-0.145, -0.155, 0.27), (0.18, 0.020, 0.08), mats["accent"], root, "Foot.R", bevel=0.008)
    add_cube("BanditRightBootWrap", (0.145, -0.155, 0.27), (0.18, 0.020, 0.08), mats["accent"], root, "Foot.L", bevel=0.008)

    # Hood + fitted cloth mask. The original generic nose/mouth stay buried inside
    # the head; the mask itself slightly intersects the face volume.
    add_hair_cap(root, mats, style="ponytail")
    add_cube("BanditHoodBack", (0.0, 0.145, 1.82), (0.47, 0.18, 0.42), mats["primary"], root, "Head", bevel=0.055)
    add_ico("BanditHoodTop", (0.0, -0.005, 1.91), (0.50, 0.42, 0.32), mats["primary"], root, "Head")
    add_cube("BanditHoodLeftFlap", (-0.220, -0.050, 1.74), (0.095, 0.15, 0.30), mats["primary"], root, "Head", bevel=0.018, rotation=(0.0, 0.0, math.radians(-12)))
    add_cube("BanditHoodRightFlap", (0.220, -0.050, 1.74), (0.095, 0.15, 0.30), mats["primary"], root, "Head", bevel=0.018, rotation=(0.0, 0.0, math.radians(12)))
    add_cube("BanditMask", (0.0, -0.205, 1.640), (0.31, 0.050, 0.160), mats["accent"], root, "Head", bevel=0.018)
    add_cube("BanditHeadWrap", (0.0, -0.198, 1.855), (0.38, 0.030, 0.070), mats["accent"], root, "Head", bevel=0.010)
    # The wrap tail starts from the right edge of the head wrap, then angles back
    # across the temple. It overlaps the wrap by several millimetres instead of
    # being authored as a disconnected block behind the head.
    add_cube("BanditWrapTail", (0.250, -0.130, 1.835), (0.20, 0.080, 0.060), mats["accent"], root, "Head", bevel=0.008, rotation=(0.0, 0.0, math.radians(26)))

    # Cross-body loot strap and a genuinely suspended-from-belt pouch. The strap
    # bridges belt and pouch; it is not an invisible gap between two loose boxes.
    add_cylinder_between("BanditChestStrap", (-0.165, -0.176, 1.43), (0.165, -0.188, 0.88), 0.028, mats["dark"], root, "Chest", vertices=6)
    add_cube("BanditPouchMountStrap", (0.250, -0.155, 0.870), (0.085, 0.055, 0.105), mats["dark"], root, "Hips", bevel=0.010)
    add_cube("BanditHipPouch", (0.335, -0.155, 0.83), (0.14, 0.070, 0.16), mats["dark"], root, "Hips", bevel=0.016)
    # Pouch front is y=-0.190. The flap rear overlaps it by 2 mm, so it reads as
    # a hinged lid rather than a detached plate 33 mm in front of the pouch.
    add_cube("BanditHipPouchFlap", (0.335, -0.198, 0.875), (0.12, 0.020, 0.070), mats["accent"], root, "Hips", bevel=0.010)
    return root


def build_bruiser_rider(team_name):
    root = create_root("FTF_Enemy_BruiserRider")
    mats = create_role_materials("BruiserRider", team_name)
    add_male_body(root, mats, mats["primary"], mats["dark"], mats["dark"], sleeve_material=mats["primary"])
    remove_standard_brows(root)

    add_cube("BruiserVestFront", (0.0, -0.205, 1.21), (0.60, 0.044, 0.50), mats["primary"], root, "Chest", bevel=0.038)
    add_cube("BruiserChestHarness", (0.0, -0.237, 1.21), (0.12, 0.016, 0.44), mats["accent"], root, "Chest", bevel=0.010)
    add_cube("BruiserShoulderPadLeft", (-0.402, -0.020, 1.38), (0.18, 0.15, 0.14), mats["dark"], root, "UpperArm.R", bevel=0.022)
    add_cube("BruiserShoulderPadRight", (0.402, -0.020, 1.38), (0.18, 0.15, 0.14), mats["dark"], root, "UpperArm.L", bevel=0.022)
    add_open_box_band_shell("BruiserRiderBelt", 0.90, 0.272, 0.164, 0.10, mats["accent"], root, "Hips")
    add_cube("BruiserBuckle", (0.0, -0.176, 0.90), (0.13, 0.018, 0.090), mats["team"], root, "Hips", bevel=0.014)
    add_cube("LeftKnuckleGuard", (-0.620, -0.020, 0.96), (0.19, 0.15, 0.14), mats["dark"], root, "Hand.R", bevel=0.020)
    add_cube("RightKnuckleGuard", (0.620, -0.020, 0.96), (0.19, 0.15, 0.14), mats["dark"], root, "Hand.L", bevel=0.020)
    # Rear side overlaps each trouser leg by 8 mm. It now reads as a strapped-on
    # guard rather than a thin plate floating 24 mm in front of the knee.
    add_cube("LeftKneeGuard", (-0.170, -0.137, 0.47), (0.16, 0.040, 0.13), mats["accent"], root, "Shin.R", bevel=0.010)
    add_cube("RightKneeGuard", (0.170, -0.137, 0.47), (0.16, 0.040, 0.13), mats["accent"], root, "Shin.L", bevel=0.010)
    add_cube("BruiserBackTank", (0.0, 0.188, 1.14), (0.28, 0.13, 0.34), mats["dark"], root, "Chest", bevel=0.028)
    add_cylinder_between("BruiserTankPipeLeft", (-0.08, 0.145, 1.25), (-0.26, 0.020, 1.29), 0.022, mats["team"], root, "Chest", vertices=6)
    add_cylinder_between("BruiserTankPipeRight", (0.08, 0.145, 1.25), (0.26, 0.020, 1.29), 0.022, mats["team"], root, "Chest", vertices=6)

    # Bald head: one heavy brow band and a properly face-contacting chin guard.
    add_cube("BruiserBrowBand", (0.0, -0.205, 1.865), (0.29, 0.040, 0.045), mats["accent"], root, "Head", bevel=0.010)
    add_cube("BruiserChinGuard", (0.0, -0.205, 1.590), (0.18, 0.040, 0.07), mats["accent"], root, "Head", bevel=0.008)
    return root


def build_future_soldier(team_name):
    root = create_root("FTF_Enemy_FutureSoldier")
    mats = create_role_materials("FutureSoldier", team_name)
    add_male_body(root, mats, mats["dark"], mats["dark"], mats["dark"], sleeve_material=mats["dark"])
    # The generic tongue is retained for the required facial shape keys, but is
    # recolored blue and fully enclosed by the helmet rather than staying Enemy red.
    replace_mesh_material(root, "FunnyTongue", mats["accent"])

    # Black/navy armor and blue technology details. No Enemy-red mesh is used by
    # this character. All plates touch/intersect their intended base volume.
    add_cube("FutureChestPlate", (0.0, -0.208, 1.22), (0.59, 0.050, 0.48), mats["primary"], root, "Chest", bevel=0.034)
    add_cube("FutureChestCore", (0.0, -0.245, 1.22), (0.18, 0.018, 0.17), mats["accent"], root, "Chest", bevel=0.010)
    add_cube("FutureAbPlate", (0.0, -0.176, 0.98), (0.42, 0.036, 0.18), mats["accent"], root, "Hips", bevel=0.012)
    add_open_box_band_shell("FutureBattleBelt", 0.90, 0.274, 0.164, 0.10, mats["primary"], root, "Hips")
    add_cube("FutureLeftShoulder", (-0.395, -0.010, 1.39), (0.17, 0.16, 0.14), mats["accent"], root, "UpperArm.R", bevel=0.020)
    add_cube("FutureRightShoulder", (0.395, -0.010, 1.39), (0.17, 0.16, 0.14), mats["accent"], root, "UpperArm.L", bevel=0.020)
    # The two penetrating black forearm blocks were deliberately removed.
    add_cube("FutureLeftShinGuard", (-0.170, -0.137, 0.47), (0.15, 0.040, 0.16), mats["primary"], root, "Shin.R", bevel=0.010)
    add_cube("FutureRightShinGuard", (0.170, -0.137, 0.47), (0.15, 0.040, 0.16), mats["primary"], root, "Shin.L", bevel=0.010)
    add_cube("FuturePowerPack", (0.0, 0.185, 1.15), (0.30, 0.12, 0.38), mats["primary"], root, "Chest", bevel=0.024)
    add_cylinder_between("FuturePackCableLeft", (-0.10, 0.145, 1.25), (-0.22, -0.010, 1.10), 0.020, mats["accent"], root, "Chest", vertices=6)
    add_cylinder_between("FuturePackCableRight", (0.10, 0.145, 1.25), (0.22, -0.010, 1.10), 0.020, mats["accent"], root, "Chest", vertices=6)

    # The formerly floating red visor/crest pair is removed. The helmet now uses
    # a simple black/navy shell with blue jaw and ear modules that overlap the shell.
    add_ico("FutureHelmetShell", (0.0, 0.010, 1.77), (0.53, 0.45, 0.48), mats["primary"], root, "Head")
    add_cube("FutureHelmetJaw", (0.0, -0.185, 1.59), (0.33, 0.10, 0.16), mats["accent"], root, "Head", bevel=0.016)
    add_cube("FutureEarLeft", (-0.268, -0.030, 1.77), (0.06, 0.10, 0.14), mats["accent"], root, "Head", bevel=0.010)
    add_cube("FutureEarRight", (0.268, -0.030, 1.77), (0.06, 0.10, 0.14), mats["accent"], root, "Head", bevel=0.010)
    return root


ROLE_BUILDERS = {
    "MaskedBandit": build_masked_bandit,
    "BruiserRider": build_bruiser_rider,
    "FutureSoldier": build_future_soldier,
}
...
# =============================================================================
# VISUAL ATTACHMENT / FLOATING-COMPONENT AUDIT
# =============================================================================
# This audit complements the coplanar triangle check. It does not forbid a small
# intentional garment offset, but rejects authoring mistakes where a decorative
# plate, pouch, mask or armor piece is visibly detached from the thing it should
# be mounted to. The 7 mm limit is below the visual scale where these low-poly
# characters read as "floating" at gameplay camera distance.
CONTACT_GAP_LIMIT = 0.007

# (source_mesh, target_meshes, require_contact_with_every_target, max_gap_m)
ATTACHMENT_RULES = {
    "MaskedBandit": (
        ("BanditJacketFront", ("Torso",), False, 0.007),
        ("BanditJacketZip", ("BanditJacketFront",), False, 0.007),
        ("BanditWaistBelt", ("Waist",), False, 0.007),
        ("BanditBuckle", ("BanditWaistBelt",), False, 0.007),
        ("BanditLeftBootWrap", ("LeftBoot",), False, 0.007),
        ("BanditRightBootWrap", ("RightBoot",), False, 0.007),
        ("BanditMask", ("Head",), False, 0.007),
        ("BanditHeadWrap", ("Head",), False, 0.007),
        ("BanditWrapTail", ("BanditHeadWrap",), False, 0.007),
        ("BanditHoodBack", ("Head",), False, 0.007),
        ("BanditHoodTop", ("Head",), False, 0.007),
        ("BanditHoodLeftFlap", ("Head",), False, 0.007),
        ("BanditHoodRightFlap", ("Head",), False, 0.007),
        ("BanditChestStrap", ("BanditJacketFront",), False, 0.010),
        ("BanditPouchMountStrap", ("BanditWaistBelt", "BanditHipPouch"), True, 0.007),
        ("BanditHipPouch", ("BanditPouchMountStrap",), False, 0.007),
        ("BanditHipPouchFlap", ("BanditHipPouch",), False, 0.007),
    ),
    "BruiserRider": (
        ("BruiserVestFront", ("Torso",), False, 0.007),
        ("BruiserChestHarness", ("BruiserVestFront",), False, 0.007),
        ("BruiserShoulderPadLeft", ("LeftUpperArm",), False, 0.010),
        ("BruiserShoulderPadRight", ("RightUpperArm",), False, 0.010),
        ("BruiserRiderBelt", ("Waist",), False, 0.007),
        ("BruiserBuckle", ("BruiserRiderBelt",), False, 0.007),
        ("LeftKnuckleGuard", ("LeftHand",), False, 0.010),
        ("RightKnuckleGuard", ("RightHand",), False, 0.010),
        ("LeftKneeGuard", ("LeftLeg",), False, 0.007),
        ("RightKneeGuard", ("RightLeg",), False, 0.007),
        ("BruiserBackTank", ("Torso",), False, 0.007),
        ("BruiserBrowBand", ("Head",), False, 0.007),
        ("BruiserChinGuard", ("Head",), False, 0.007),
    ),
    "FutureSoldier": (
        ("FutureChestPlate", ("Torso",), False, 0.007),
        ("FutureChestCore", ("FutureChestPlate",), False, 0.007),
        ("FutureAbPlate", ("Waist",), False, 0.007),
        ("FutureBattleBelt", ("Waist",), False, 0.007),
        ("FutureLeftShoulder", ("LeftUpperArm",), False, 0.010),
        ("FutureRightShoulder", ("RightUpperArm",), False, 0.010),
        ("FutureLeftShinGuard", ("LeftLeg",), False, 0.007),
        ("FutureRightShinGuard", ("RightLeg",), False, 0.007),
        ("FuturePowerPack", ("Torso",), False, 0.007),
        ("FutureHelmetShell", ("Head",), False, 0.007),
        ("FutureHelmetJaw", ("FutureHelmetShell",), False, 0.007),
        ("FutureEarLeft", ("FutureHelmetShell",), False, 0.007),
        ("FutureEarRight", ("FutureHelmetShell",), False, 0.007),
    ),
}


def semantic_mesh_name(obj):
    """Return a stable name in both authored and glTF-reimported scenes."""
    for candidate in (obj.name, obj.data.name if getattr(obj, "data", None) else ""):
        if candidate.startswith("MESH_"):
...
def mesh_lookup(meshes):
    lookup = {}
    for mesh in meshes:
        key = semantic_mesh_name(mesh)
        if key in lookup:
            raise RuntimeError("Attachment audit requires unique mesh names, duplicate: " + key)
        lookup[key] = mesh
    return lookup
...
def aabb_gap_meters(first, second):
    """Euclidean gap between two world-space AABBs; 0 means they overlap/touch."""
    a = world_aabb(first)
    b = world_aabb(second)
...
def role_from_asset_name(asset_name):
    for role_name in ATTACHMENT_RULES:
        if role_name in asset_name:
            return role_name
    return None


def validate_attachment_contacts(meshes, role_name, stage):
    """Fail on named parts whose intended mounting relation visibly separates."""
    if role_name not in ATTACHMENT_RULES:
        return
    lookup = mesh_lookup(meshes)
    failures = []
    for source_name, target_names, require_all, max_gap in ATTACHMENT_RULES[role_name]:
        source = lookup.get(source_name)
        if source is None:
            failures.append("missing audited source " + source_name)
            continue
        target_pairs = []
        for target_name in target_names:
            target = lookup.get(target_name)
            if target is None:
                failures.append("missing audited target %s for %s" % (target_name, source_name))
                continue
            target_pairs.append((target_name, aabb_gap_meters(source, target)))
        if not target_pairs:
            continue
        if require_all:
            rejected = [(name, gap) for name, gap in target_pairs if gap > max_gap]
            if rejected:
                details = ", ".join("%s=%.1fmm" % (name, gap * 1000.0) for name, gap in rejected)
                failures.append("detached %s from required mounts: %s (limit %.1fmm)" % (source_name, details, max_gap * 1000.0))
        else:
            nearest_name, nearest_gap = min(target_pairs, key=lambda item: item[1])
            if nearest_gap > max_gap:
                details = ", ".join("%s=%.1fmm" % (name, gap * 1000.0) for name, gap in target_pairs)
                failures.append("detached %s: nearest allowed mount is %s at %.1fmm; all=%s (limit %.1fmm)" % (
                    source_name, nearest_name, nearest_gap * 1000.0, details, max_gap * 1000.0
                ))
    if failures:
        raise RuntimeError("Visual attachment audit failed at %s: %s" % (stage, "; ".join(failures)))
    print("ATTACHMENT AUDIT OK %s %s: all named visual parts are connected within %.1fmm" % (
        stage, role_name, CONTACT_GAP_LIMIT * 1000.0
    ))
...
def validate_character(root, armature, meshes):
    ...
    validate_attachment_contacts(meshes, role_from_asset_name(root.name), "build")
...
def verify_export(path):
    ...
    validate_attachment_contacts(meshes, role_from_asset_name(os.path.basename(path)), "reimport")
...
def build_export_validate_variant(role_name, team_name, output_dir, render_checks):
    ...
    output_path = os.path.join(output_dir, f"FTF_Enemy_{role_name}_Rigged_V1_5.glb") if team_name == "Enemy" else os.path.join(output_dir, f"FTF_Character_{role_name}_{team_name}_Rigged_V1_5.glb")
...
def main():
    ...
    output_paths = []
    for role_name in ("MaskedBandit", "BruiserRider", "FutureSoldier"):
        output_paths.append(
    ...
    print(
        "DONE: generated %d animated enemy GLBs (MaskedBandit/BruiserRider/FutureSoldier) in %s"
