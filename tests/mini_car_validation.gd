...
func _run() -> void:
	var root := Node3D.new()
	root.name = "MiniCarValidationRoot"
	add_child(root)
	var packed := load("res://vehicles/mini_car.tscn") as PackedScene
	_check(packed != null, "mini car scene loads")
	if packed == null:
		_finish(root)
		return
	var vehicle := packed.instantiate() as VehicleBase
	_check(vehicle != null, "mini car scene instantiates as VehicleBase")
	if vehicle == null:
		_finish(root)
		return
	...
	_check(vehicle is CharacterBody3D, "mini car root is CharacterBody3D")
	_check(vehicle.get_seat_count() == 2, "mini car has one driver and one passenger seat")
	_check(vehicle.get_driver_seat_index() == 0, "driver seat is seat zero")
	_check(vehicle.seat_can_drive(0), "driver seat can drive")
	_check(not vehicle.seat_can_drive(1), "passenger seat cannot drive")
	_check(is_equal_approx(vehicle.vehicle_config.max_hp, 600.0), "mini car has 600 HP")
	_check(is_equal_approx(vehicle.current_hp, 600.0), "mini car starts at 600 HP")
	_check(is_equal_approx(vehicle.get_max_forward_speed(), 6.0), "mini car maximum speed is 6 m/s")
	_check(is_equal_approx(vehicle.get_camera_fov_for_speed(6.0), 94.0), "mini car reaches its configured top-speed FOV")
	_check(not vehicle.supports_cargo(), "mini car does not support cargo")
	_check(vehicle.is_in_group("vehicle_bases"), "mini car registers as a vehicle")
	_check(vehicle.get_node_or_null("GroundProbe") is RayCast3D, "ground probe exists")
	_check(vehicle.get_node_or_null("DriverSeat") is Node3D, "driver seat anchor exists")
	_check(vehicle.get_node_or_null("PassengerSeat1") is Node3D, "passenger seat anchor exists")
...
func _finish(root: Node) -> void:
	...
	if failures.is_empty():
		print("[MiniCarValidation] PASS")
		get_tree().quit(0)
	else:
		for failure: String in failures:
			push_error("[MiniCarValidation] " + failure)
		get_tree().quit(1)
